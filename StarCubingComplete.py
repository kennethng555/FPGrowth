# -*- coding: utf-8 -*-
"""
Created on Sat Nov  2 23:24:12 2019
            ...
            for j in range(num_dim-1):
                if(self.head.get_left() == None):
                    ...
                    if self.head.get_right() == None and self.head.get_data() != df.iloc[i,j]:
                        new_right = Node(data=df.iloc[i,j], count=df.iloc[i,num_dim], parent = self.head.get_parent(), depth = j)
    ...
    def starcubing(self, t, cnode):
        ...
        while self.c_tree.neighbor is not None:
            ...
            if self.c_tree.neighbor.head.depth > cnode.depth:
                ...
                #print(separator.join([str(self.c_tree.neighbor.root.get_data()), str(self.c_tree.neighbor.root.get_count())]), end = '\t')
                if self.c_tree.neighbor.head.depth == cnode.depth:
                    if  cnode.data == self.c_tree.neighbor.head.data or self.getpath(cnode) == self.c_tree.neighbor.head.data:
                        ...
                        new_node = Node(cnode.data, cnode.count, parent=self.c_tree.neighbor.head.parent, depth=cnode.depth)
...
def decode(startable):
    ...
    for i in range(len(startable)):
        for j in range(len(startable.columns)-1):
            startable.iloc[i,j] = separator.join([separator2.join([chr(97+math.floor(j/26)) * (math.floor(j/26)), chr(97+(j%26))]), str(int(startable.iloc[i,j]))])
    return startable
